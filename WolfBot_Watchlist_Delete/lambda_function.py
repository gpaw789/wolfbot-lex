...
def delete_symbol(userid, symbol):
    ...
    try:
        ...
        if symbol in symbol_list:
            position = symbol_list.index(symbol)       #get the index
            response = table.update_item(
                Key={
                    'userid': userid,
                },
                UpdateExpression="REMOVE symbol_list[{}]".format(position),  # delete the index element
...
def action_WolfBot_Watchlist_Delete(intent_request):
    """
    ...
    #get the code, 0 = success, 1 = error
    success_code = delete_symbol(intent_request["userId"], symbol)

    #building content
    if success_code == 0:
        content = "Deleted {} from your watchlist. " \
                    "Type \"Show me my watchlist\".".format(symbol)
    else:
        content = "Failed to delete {} from your watchlist. Please try again or type \"Help Me\".".format(symbol)
...
def dispatch(intent_request):
    ...
    if intent_name == 'WolfBot_Watchlist_Delete':
        return action_WolfBot_Watchlist_Delete(intent_request)
